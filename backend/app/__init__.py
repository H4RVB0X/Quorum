...
def create_app(config_class=Config):
    ...
    # Dashboard route — serves dashboard.html directly from Flask
    @app.route('/dashboard')
    def dashboard():
        import os
        from flask import send_file, Response
        dashboard_path = os.path.join(os.path.dirname(__file__), '../../frontend/public/dashboard.html')
        if os.path.exists(dashboard_path):
            return send_file(dashboard_path)
        return Response("Dashboard not found. Place dashboard.html in frontend/public/", status=404)
